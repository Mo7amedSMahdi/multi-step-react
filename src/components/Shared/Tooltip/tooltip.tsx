 ...
 * @param duration - The time in milliseconds that the tooltip will be displayed. If not
 *  provided, the tooltip will be displayed until the user clicks outside of it.
 ...
 */
export const Tooltip: React.FC<Props> = function ({
  label,
  className,
  ...
  return (
    <>
      {children ? (
        ...
        <button
          ...
          onClick={(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
            e.stopPropagation();
            const floatingUIOnClick = onClick as (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => void;
            floatingUIOnClick(e);
      ...
      <FloatingPortal>
        {open && (
          <div
            className={cn('z-50 max-w-[300px] rounded border border-marble-400 bg-marble-200 px-4 py-2.5', className)}
            ...
            {...getFloatingProps()}
          >
            <Text className={cn('whitespace-normal text-volcanic-800', textClassname)}>{label}</Text>
          </div>
        )}
      </FloatingPortal>
    </>
